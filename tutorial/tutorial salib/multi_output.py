...
if __name__ == "__main__":
    ...
    (sp.sample_saltelli(1024).evaluate(lake_problem.evaluate).analyze_sobol(nprocs=2))
    ...
    sp = ProblemSpec(
        {"names": ["x1", "x2", "x3"], "bounds": [[-np.pi, np.pi]] * 3, "outputs": ["Y"]}
    )

    (sp.sample_saltelli(512).evaluate(Ishigami.evaluate).analyze_sobol())
